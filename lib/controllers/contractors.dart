...
class ContractorsController extends GetxController {
  ...
  RxList<INSSCustomer> contractors = <INSSCustomer>[].obs;
  ...
  Future<void> listContractors({bool paginated = false}) async {
    if (paginated) {
    ...
    Map<String, String> filters = geFiltersContractors();
    ...
    saveContractors(customers, paginated);
    loading.value = false;
  }

  saveContractors(List<INSSCustomer> data, bool paginated) {
    if (!paginated) {
      contractors.value = data;
      currentPage += 1;
    } else {
      contractors.addAll(data);
      currentPage += 1;
      update();
    }
  }

  Map<String, String> geFiltersContractors() {
    Map<String, String> queryParams = {};
    queryParams['insured_type'] = 'contractor';
